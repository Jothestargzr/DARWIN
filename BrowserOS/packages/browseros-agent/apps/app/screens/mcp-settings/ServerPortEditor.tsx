...
async function readCurrentPort(): Promise<number> {
  try {
    ...
    // Pref unset or DARWIN API unavailable — fall back to the default port
    return PROXY_PORT_MIN
  }
}

/** Popover for editing the MCP proxy port and waiting for DARWIN to rebind it. */
export const ServerPortEditor: FC<ServerPortEditorProps> = ({
  ...
  const handleSave = async () => {
    ...
    setError(null)
    try {
      const success = await getDARWINAdapter().setPref(
