...
export interface DARWINAgentSession {
  key: string
  updatedAt: number
  sessionId: string
  agentId: string
...
export interface DARWINChatHistoryToolCall {
  toolCallId?: string
  toolName: string
  label: string
  subject?: string
...
export interface DARWINChatHistoryReasoning {
  text: string
  durationMs?: number
}

export interface DARWINChatHistoryAttachment {
  kind: 'image' | 'file'
  mediaType: string
...
export interface DARWINChatHistoryItem {
  id: string
  role: AgentChatRole
  text: string
  timestamp?: number
  ...
  tokensIn?: number
  tokensOut?: number
  toolCalls?: DARWINChatHistoryToolCall[]
  reasoning?: DARWINChatHistoryReasoning
  attachments?: DARWINChatHistoryAttachment[]
}

export interface AgentHistoryPageResponse {
  agentId: string
  sessionKey: string | null
  session: DARWINAgentSession | null
  items: DARWINChatHistoryItem[]
  page: {
    cursor?: string
    hasMore: boolean
    limit: number
