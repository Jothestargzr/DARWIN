...
function parseErrorMessage(
  ...
  // All chat requests go through the local DARWIN agent server, so any
  // fetch failure is always a local connection issue.
  if (message.includes('Failed to fetch') || message.includes('fetch failed')) {
    return {
      text: 'Unable to connect to DARWIN agent. Follow below instructions.',
      url: 'https://docs.browseros.com/troubleshooting/connection-issues',
      isConnectionError: true,
    }
  }

  // Detect credit exhaustion from gateway (DARWIN provider only)
  if (
    isBrowserosProvider &&
  ...
  // Detect DARWIN rate limit (DARWIN provider only)
  if (
    isBrowserosProvider &&
    message.includes('DARWIN LLM daily limit reached')
  ) {
    return {
