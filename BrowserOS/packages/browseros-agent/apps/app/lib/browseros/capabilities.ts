...
type FeatureConfig = {
  minDARWINVersion?: string
  maxDARWINVersion?: string
...
/**
 * Features gated by DARWIN version or explicit environment flags.
 ...
 * - minDARWINVersion: feature enabled when DARWIN >= this version
 * - maxDARWINVersion: feature enabled when DARWIN < this version (for deprecation)
...
const FEATURE_CONFIG: { [K in Feature]: FeatureConfig } = {
  ...
  [Feature.NEWTAB_CHAT_SUPPORT]: { minDARWINVersion: '0.40.0.0' },
  [Feature.VERTICAL_TABS_SUPPORT]: { minDARWINVersion: '0.42.0.0' },
  ...
  [Feature.AGENT_HARNESS_SUPPORT]: { minDARWINVersion: '0.46.0.0' },
}

function hasVersionConstraints(config: FeatureConfig): boolean {
  return Boolean(
    config.minDARWINVersion ||
      config.maxDARWINVersion ||
...
async function doInitialize(): Promise<CapabilitiesState> {
  const adapter = DARWINAdapter.getInstance()
  ...
  try {
    ...
    // DARWIN version unknown - features requiring it will be disabled
  }
...
export function checkFeatureSupport(
  ...
  const hasDARWINConstraints =
    config.minDARWINVersion || config.maxDARWINVersion
  if (
    hasDARWINConstraints &&
...
export const Capabilities = {
  ...
  async getDARWINVersion(): Promise<string | null> {
    const state = await ensureInitialized()
