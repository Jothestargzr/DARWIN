...
describe('side panel scope routing', () => {
  ...
  it('keeps toolbar toggles on the DARWIN tab-specific API when scope storage is absent', async () => {
    const result = await toggleSidePanel({ tabId: 7, windowId: 3 })
  ...
  it('keeps toolbar toggles on the DARWIN tab-specific API when scope storage is false', async () => {
  ...
  it('keeps programmatic opens on the DARWIN API in window mode', async () => {
    await setSidePanelPerWindowPreference(true)
