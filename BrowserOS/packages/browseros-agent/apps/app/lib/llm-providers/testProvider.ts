...
export async function testProvider(
  ...
  try {
    ...
    // round-trip to the local DARWIN server that hosts
    ...
    // as "DARWIN dropped the port I typed" (see issue #1844).
    ...
    return {
      success: false,
      message:
        `Could not reach the local DARWIN server at ${agentServerUrl}. ` +
        `Make sure DARWIN is running and try again. (${detail})`,
      responseTime,
    }
  }
}
