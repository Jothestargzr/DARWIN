...
/** Mirrors provider data into DARWIN prefs without blocking local writes. */
async function backupToDARWIN(backup: LlmProvidersBackup): Promise<void> {
  try {
    const adapter = getDARWINAdapter()
    await adapter.setPref(BROWSEROS_PREFS.PROVIDERS, JSON.stringify(backup))
  } catch {
    // DARWIN API not available - ignore
  }
}

/** Sets up one-way sync of LLM providers to DARWIN prefs. */
export function setupLlmProvidersBackupToDARWIN(): () => void {
  const unsubscribe = providersStorage.watch(async (providers) => {
    if (providers) {
      const defaultProviderId = await defaultProviderIdStorage.getValue()
      await backupToDARWIN({ defaultProviderId, providers })
    }
  })
  return unsubscribe
}
...
/** Creates the default DARWIN provider configuration */
export function createDefaultDARWINProvider(): LlmProviderConfig {
...
export function createDefaultProvidersConfig(): LlmProviderConfig[] {
  return [createDefaultDARWINProvider()]
}
