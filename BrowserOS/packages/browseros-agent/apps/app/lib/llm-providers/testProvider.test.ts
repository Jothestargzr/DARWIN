...
describe('testProvider — client-side fetch failure (issue #1844)', () => {
  it('wraps a network fetch error as "could not reach DARWIN server"', async () => {
    ...
    expect(result.message).toContain('local DARWIN server')
  ...
  it('does NOT wrap a server-returned failure (provider error passes through)', async () => {
    ...
    expect(result.message).not.toContain('local DARWIN server')
  })
})
